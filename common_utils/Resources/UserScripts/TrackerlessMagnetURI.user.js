...
!function(a) {
    "use strict";
    const { document, top, self } = window;
    ...
    try {
        ...
        if (!magnets.length) { /* console.log("no magnets found"); */ return; }
        ...
        var r, e, s = document?.querySelectorAll("a[href^='magnet:']");
        if (s) for (a in s) null != s[a] && null != s[a].href && (r = new URL(s[a].href)).searchParams.has("xt") && (e = "magnet:?xt=" + r.searchParams.getAll("xt"),
